...
{
    public static readonly short GOT_DAMAGE = 1;
}
...
public class EnemyController : MonoBehaviour {
	...
	void EnemyAnimationState() {
		...
		if(ctrlAnimState == ControlAnimationState.Death)
		{
			...
			if(this.gameObject.tag == "Enemy")
			{
				...
				//this.GetComponent<DropItem>().TryDropItem();
			...
			if(destinationDistance <= enemyStatus.status.atkRange){		// Reset speed to 0
				...
				if(enableRegen)
				{
                    if (regenHP)
                    {
                        ...
                        gameObject.GetComponent<Health>().GetDamage(enemyStatus.status.hp, defaultHP); 
                    }
                    if (regenMP)
	...
	{
        dispatcher.SendEvent(RoleEvent.GOT_DAMAGE);
		...
		}else
		{
			...
			if(enemyStatus.status.hp <= 0)
			{
                ...
                PlayerDataMgr.Instance.playerData.AddCoin(enemyStatus.coinGive);
				ctrlAnimState = ControlAnimationState.Death;
                //掉落
                var drop = gameObject.GetComponent<DropItem>();
                if (drop != null)
                    drop.TryDropItem();
			}else
			{
